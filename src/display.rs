...
impl<T> HexWriter<T> {
    ...
    /// in the given `case`.
    pub fn new(dest: T, case: Case) -> Self { Self(dest, case) }
    ...
    T: core::fmt::Write,
{
    ...
    fn write(&mut self, buf: &[u8]) -> Result<usize, std::io::Error> {
        ...
        for byte in buf {
            let pair = table.byte_to_hex(*byte);
            if self.0.write_str(&pair).is_err() {
                break;
            }
            n += 1;
        }
        ...
        fn hex_writer() {
            ...
            macro_rules! test_hex_writer {
                ($cap:expr, $case: expr, $src: expr, $want: expr, $hex_result: expr) => {
                    ...
                    let mut dest = HexWriter(dest_buf, $case);
